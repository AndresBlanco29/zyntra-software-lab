document.addEventListener('DOMContentLoaded', function() {
    ...
    function initPromoCountdowns() {
        ...
        tickPromoCountdowns();
        if (window.__promoCountdownTimer) {
            window.clearInterval(window.__promoCountdownTimer);
        }
        window.__promoCountdownTimer = window.setInterval(tickPromoCountdowns, 1000);
    }
    ...
    initComboBuilder({
        ...
        onSubmitComplete: function (result) {
            if (result.totalItems != null) {
                syncMyOrderAttention(result.totalItems);
            }
        },
    });
    ...
    // Promo countdowns must start even when combo cards (no presentation select) are present.
    initPromoCountdowns();
