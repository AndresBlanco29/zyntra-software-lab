...
from config.inventario.services import aplicar_verificacion_picking_inventario, reservar_stock_para_pedido_items
...
class DualLedgerEndToEndTests(TestCase):
	...
	def test_picking_does_not_change_quick_inventory(self):
	...
	def test_full_flow_order_invoice_export_import(self):
		# Paso 1: QI=20, open order 10 -> Available 10
		pedido = crear_pedido_desde_items(
			cliente=self.cliente,
			items_payload=[{'presentacion': self.presentacion, 'cantidad': 10, 'precio': Decimal('10.00')}],
			origen='CLIENTE',
		...
		# Paso 2: invoice -> pending 10, in orders 0, available still 10; QI unchanged
		aplicar_verificacion_picking_inventario(
			pedido=pedido,
		...
		# Paso 3: QB restock / import QI=35; pending remains 10 -> Available 25
		...
		# Paso 4: another invoice 20 pending -> Available 5
		...
		# Paso 5: export both invoices + import QI=5
