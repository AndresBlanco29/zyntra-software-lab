...
def backfill_suppliers(apps, schema_editor):
    ...
    for compra in CompraProveedor.objects.exclude(proveedor_nombre='').order_by('id'):
        ...
        if supplier is None:
            ProveedorModel.objects.create(nombre=nombre, **defaults)
            continue
        changed_fields = []
