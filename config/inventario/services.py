...
def _lock_stock_records(presentacion_ids):
	...
	for presentacion_id in sorted(set(presentacion_ids)):
		try:
			...
			pass
	for stock in StockPresentacion.objects.select_for_update().select_related('presentacion__producto').filter(presentacion_id__in=presentacion_ids).order_by('presentacion_id'):
...
def aplicar_verificacion_picking_inventario(*, pedido, pedido_item_ids, creado_por=None):
	items = list(PedidoItem.objects.select_for_update().select_related('pedido', 'presentacion__producto').filter(id__in=pedido_item_ids).order_by('presentacion_id', 'id'))
...
def cancelar_pedido_con_inventario(*, pedido, creado_por=None):
	items = list(PedidoItem.objects.select_for_update().select_related('presentacion__producto').filter(pedido=pedido).order_by('presentacion_id', 'id'))
	stock_map = _lock_stock_records(item.presentacion_id for item in items)
	for item in items:
		stock = stock_map[item.presentacion_id]
...
@transaction.atomic
def procesar_nota_credito_inventario(*, nota, creado_por=None):
	if nota.tipo_documento != 'CREDITO' or nota.tipo_credito != 'CREDIT_RETURN':
		return False
	items = list(nota.items.select_related('presentacion', 'nota__invoice').order_by('presentacion_id', 'id'))
	stock_map = _lock_stock_records(item.presentacion_id for item in items if item.presentacion_id)
	for item in items:
		if not item.presentacion_id:
			continue
		stock = stock_map[item.presentacion_id]
		_apply_inventory_change(
			stock=stock,
			categoria='ENTRADA',
			tipo='ENTRADA_NOTA_CREDITO',
			cantidad=item.cantidad,
			delta_fisico=item.cantidad,
			delta_reservado=0,
			referencia=nota.numero,
			idempotency_key=f'CRN-APLICA-{nota.id}-{item.id}',
			invoice=nota.invoice,
			nota_ajuste=nota,
			nota_ajuste_item=item,
			creado_por=creado_por,
		)
	return True
...
@transaction.atomic
def revertir_nota_credito_inventario(*, nota, creado_por=None):
	if nota.tipo_documento != 'CREDITO' or nota.tipo_credito != 'CREDIT_RETURN':
		return False
	items = list(nota.items.select_related('presentacion', 'nota__invoice').order_by('presentacion_id', 'id'))
	stock_map = _lock_stock_records(item.presentacion_id for item in items if item.presentacion_id)
	for item in items:
		...
		_apply_inventory_change(
			stock=stock,
			categoria='AJUSTE',
			tipo='REVERSO_NOTA_CREDITO',
			cantidad=item.cantidad,
			delta_fisico=-item.cantidad,
			delta_reservado=0,
			referencia=nota.numero,
			idempotency_key=f'CRN-REVERSO-{nota.id}-{item.id}',
			invoice=nota.invoice,
			nota_ajuste=nota,
			nota_ajuste_item=item,
			creado_por=creado_por,
...
def registrar_entrada_manual(*, presentacion, cantidad, observacion='', creado_por=None):
	...
	return _apply_inventory_change(
		...
		delta_fisico=max(int(cantidad), 1),
		delta_reservado=0,
		referencia=f'MANUAL-IN-{presentacion.id}',
		observacion=observacion,
		creado_por=creado_por,
	)
...
def registrar_salida_manual(*, presentacion, cantidad, observacion='', creado_por=None):
	...
	return _apply_inventory_change(
		...
		delta_fisico=-qty,
		delta_reservado=0,
		referencia=f'MANUAL-OUT-{presentacion.id}',
		observacion=observacion,
		creado_por=creado_por,
	)
...
def registrar_ajuste_manual(*, presentacion, delta_cantidad, observacion='', creado_por=None):
	stock = _lock_stock_records([presentacion.id])[presentacion.id]
	delta = int(delta_cantidad)
	if delta == 0:
		raise ValidationError(_('Inventory adjustments must modify at least one unit.'))
	return _apply_inventory_change(
		stock=stock,
		categoria='AJUSTE',
		...
		delta_fisico=delta,
		delta_reservado=0,
		referencia=f'MANUAL-ADJ-{presentacion.id}',
		observacion=observacion,
		creado_por=creado_por,
	)